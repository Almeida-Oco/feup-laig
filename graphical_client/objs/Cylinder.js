 ...
 */
function Cylinder(scene, args, inverted) {
...
Cylinder.prototype.initBuffers = function () {
  var t = Math.PI * 2 / this.slices,
    delta = (this.t_radius - this.b_radius) / this.stacks;
  var ang = 0;
  ...
  for (j = 0; j <= this.stacks; j++) {
    ...
    for (i = 1; i <= this.slices; i++) {
      ...
      if (j > 0 && i > 0) {
        if (this.inverted) {
          this.indices.push(verts - this.slices - 2, verts - 2, verts - 1);
          this.indices.push(verts - 2, verts - this.slices - 2, verts - this.slices - 3);
        }
        else {
